...
def setup_weather_data_crawler():
    crawler_name = 'weather_JSON_crawler'
    s3_target_path = 's3://weather-influence-on-sales/raw/weather/' # S3 path where weather data is stored
    ...
    try:
        glue.create_crawler(
            ...
            Description = 'Crawler for weather JSON data in S3 bucket to data catalog',
            Targets={
                'S3Targets': [
                    {
                        'Path': s3_target_path
